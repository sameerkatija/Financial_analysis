...
def SalesGrowth(lst):
    try:
        ...
        status = 'GOOD' if one_year_cagr > 15 else ('AVG' if one_year_cagr > 5 and one_year_cagr < 15 else 'BAD' )
        five_year_status = 'GOOD' if five_year_cagr > 15 else ('AVG' if five_year_cagr > 5 and five_year_cagr < 15 else 'BAD' )
...
def NetProfitGrowth(lst):
    try:
        ...
        five_year_cagr = round((((lst.iloc[0] / lst.iloc[4])**(1/5)) -1) * 100,2)
        status = 'GOOD' if one_year_cagr > 15 else ('AVG' if one_year_cagr > 5 and one_year_cagr < 15 else 'BAD' )
        five_year_status = 'GOOD' if five_year_cagr > 15 else ('AVG' if five_year_cagr > 0 and five_year_cagr < 15 else 'BAD' )
